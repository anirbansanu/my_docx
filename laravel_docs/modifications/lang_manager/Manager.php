<?php
//  this file is overridden for Themsaid\Langman\Manager;
namespace App\Overrides\Langman;
...
class Manager
{
    ...
    public function getJsonFilesContent($filesByFile,$lang)
    {
        ...
        foreach ($filesByFile as $fileName => $value) {
            if (array_key_exists($lang,$filesByFile[$fileName])) // check is language avl 
            {
                ...
                $data[$fileName][$lang] = json_decode($contents, true);       // data['customer_app']['en'] = array of json file content
            }
        }
        return $data;
    ...
    public function genarateLang($lang_name){
        $default_lang = setting('language');
        $sourcePath = resource_path("lang/$default_lang"); // Path to the source language folder
        $destinationPath = resource_path("lang/$lang_name"); // Path to the destination language folder
        if(!File::exists($sourcePath)) {
            return 0;
        }
        
        // Check if the destination directory already exists, if not, create it and set permissions
        if (!File::exists($destinationPath)) {
            File::makeDirectory($destinationPath, 0777, true); // 0755 sets the folder permissions to rwxr-xr-x
        }

        // Perform the copy operation and handle exceptions
        try {
            File::copyDirectory($sourcePath, $destinationPath);
            File::chmod($destinationPath, 0777, true);
            return 1;
        } catch (Exception $e) {
